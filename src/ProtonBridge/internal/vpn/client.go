...
// GetMaxTier reads the authenticated account entitlement; the logical list alone
// includes servers the account is not allowed to use. Added by discSapo.
func (c *Client) GetMaxTier() (int, error) {
	var response struct {
		Code int
		VPN  struct{ MaxTier *int }
	}
	if err := c.doJSON(http.MethodGet, c.config.APIURL+"/vpn/v2", nil, &response); err != nil {
		return 0, err
	}
	if !constants.IsSuccessCode(response.Code) || response.VPN.MaxTier == nil || *response.VPN.MaxTier < 0 {
		return 0, fmt.Errorf("account entitlement unavailable")
	}
	return *response.VPN.MaxTier, nil
}
