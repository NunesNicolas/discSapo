...
func (c *Client) performFreshAuth() (*api.Session, error) {
	...
	// discSapo only implements TOTP. Leave hardware challenges to the portal.
	if authInfo.TwoFA.Enabled == constants.EnabledTrue && authInfo.TwoFA.TOTP != constants.EnabledTrue {
		return nil, fmt.Errorf("security key verification requires the portal")
	}
...
func (c *Client) get2FACode() (string, error) {
	if c.config.RequestTwoFactor != nil {
		return c.config.RequestTwoFactor()
	}
	fmt.Print("2FA Code: ")
	reader := bufio.NewReader(os.Stdin)
