...
// Build returns the configuration over private IPC without writing a plaintext file.
// Added by discSapo.
func (g *ConfigGenerator) Build(server *api.LogicalServer, physical *api.PhysicalServer, key string, info *api.VPNInfo) (string, error) {
	return g.buildConfig(server, physical, key, info)
}
